...
export const Editor = ({ webId }: Props) => {
  ...
  async function setUrlFromStorage() {
    if (webId) {
      ...
      if (storageRoot) {
        ...
        const ACLFolder = new AccessControlList(webId, new URL(folderName + '/', exampleUrl));
        let permissions = await ACLFolder.getPermissions()
        let users = [].concat.apply([], permissions.filter(p => p.modes.includes("Read")).map(p => p.agents))
  ...
  function handleFileChange(event) {
    ...
    setFileURL(new URL("/" + folderName + "/" + event.target.files[0].name, baseURL))
  }
  ...
  async function handleSave(event) {
    ...
    reader.onload = async f => {
      ...
      const result = await SolidAuth.fetch(baseURL + folderName + "/" + file.name, {
        method: 'PUT',
        body: data,
        headers: {
      ...
      if (result.ok) {
        ...
        const ACLFolder = new AccessControlList(webId, new URL(folderName + '/', baseURL));
  ...
  async function getFileDetails(fileURL) {
    ...
    let filesDetails = await doc
      .then(async response => {
        ...
        console.log("wac")
        return {hasPermission: wacAllowModes.user.read, url: fileURL, value: text};
      })
      .catch(() => {
  ...
  return (
    <Form>
      ...
      <FullGridSize>
        <Label>
          File
          <Input type="file" size="200" value={fileName} onChange={handleFileChange} />
        </Label>
        <Label>
          Users (separated by ',')
          <Input type="text" size="200" value={users} onChange={handleUsersChange} />
        </Label>
        <div className="input-wrap">
          <Button className="ids-link-filled ids-link-filled--primary button" onClick={handleSave}>
            {t('editor.save')}
          </Button>
        </div>
      </FullGridSize>
      <FullGridSize>
        <Label>
          Photo Owner
          <Input type="text" size="200" value={photoOwner} onChange={handlePhotoOwnerChange} />
        </Label>
        <div className="input-wrap">
          <Button className="ids-link-filled ids-link-filled--primary button" onClick={handleSearch}>
            Search
          </Button>
        </div>
        <Table>
